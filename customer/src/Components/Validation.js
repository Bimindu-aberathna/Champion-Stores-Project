...
export function validatePassword(password) {
  //allows only valid passwords.Contains at least one number, one special character and one alphabet. Minimum length is 7
...
export const validateExpiryDate = (expiry) => {
  ...
  if (expiryDate < currentDate) {
    return {
      status: false,
      message: "Invalid expiry date. The date must be in the future.",
    };
  }
...
export const validateCardNumber = (number) => {
  ...
  if (!cardNumberRegex.test(numberWithoutSpaces)) {
    return {
      status: false,
      message:
        "Invalid card number. Please enter a valid 13-16 digit card number.",
    };
  }
