    ...
    db.query(sql, (err, result) => {
        ...
        if (result.length > 0) {
            ...
            bcrypt.hash(password,10).then((hash) => {
                ...
                const values = [firstName, lastName, email, hash, address, phoneNumber];
        ...
        if (result.length > 0) {
            ...
            bcrypt.compare(password, dbPassword).then((match) => {
                if (match) {
                    ...
                    return res.status(200).json({ auth:true,message: "Login successful",accessToken:accessToken,customerName:result[0].firstName,customerID: result[0].customerID});
                } else {
